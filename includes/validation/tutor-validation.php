<?php
/**
 * Validation functions for tutor registration
 */

function validateTutorRegistration($post, $files) {
    $errors = [];
    ...
    if (empty($post['subjects']) || !is_array($post['subjects'])) {
        $errors[] = "Veuillez sélectionner au moins une matière.";
    } elseif (count($post['subjects']) > 5) {
        $errors[] = "Vous ne pouvez sélectionner que 5 matières maximum.";
    }
